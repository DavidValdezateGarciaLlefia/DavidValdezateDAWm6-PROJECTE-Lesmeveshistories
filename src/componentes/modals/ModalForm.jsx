...
export default function ModalForm({ isOpen, onClose, initialData }) {
    ...
    const [isEditing, setIsEditing] = useState(initialData && initialData.id ? true : false);

    useEffect(() => {
        setFormData(initialData);
        setIsEditing(initialData && initialData.id ? true : false); 
    }, [initialData]);
        ...
        <Modal isOpen={isOpen} onClose={onClose} placement="center">
            <ModalContent>
                <form onSubmit={controladorEnvio}>
                    <ModalHeader>
                        {isEditing ? `Editar historia: ${formData.titulo}` : `Crear nueva historia: ${formData.titulo || ''}`}
                    </ModalHeader>
                    <ModalBody>
                        <Input
                            clearable
